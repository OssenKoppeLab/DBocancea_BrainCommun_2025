...
dat <- dat %>% 
  select(CAV_1, CAV_2, CAV_3, CAV_4, CAV_5, CAV_6, CAV_7, CAV_8, CAV_9, CAV_10, CAV_11, CAV_12, CAV_13, CAV_14, CAV_15,
         ...
         MMSE_q, Z_MEM, Z_EXEC, temporoparietal_thick_weighted,
...
GGally::ggcorr(dat[,c(FAV_Q_items, "FAV_total")],
               method = c("pairwise.complete.obs", "spearman"),
               nbreaks = 6,
               label = TRUE,
               label_size = 3,
...
If I were to do a complete case on the selected dataset - I remain with 596/904 = 65% remaining, 35% missing 

```{r}

dat2 <- dat %>%
  select(CAV_lifetime, CAV_past, CAV_current, FAV_total,
         I_ID, I_sex, I_age_q, I_edu_VE, APOE, D_Q_grouped,
         MMSE_q, Z_MEM, Z_EXEC, temporoparietal_thick_weighted)

dat_complete <- dat2[complete.cases(dat2),]
...
CAV_items:  sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, cav_items, fav_total_passive
FAV_items:  sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, fav_items, cav_past_passive, cav_current_passive

I_Edu_VE :  sex, age,      apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, fav_total_passive, cav_past_passive, cav_current_passive
APOE :      sex, age, edu,       temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, fav_total_passive, cav_past_passive, cav_current_passive
a_positive: sex, age, edu, apoe,                                 diagnosis, mmse, mem, exec, fav_total_passive, cav_past_passive, cav_current_passive
MMSE:       sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis,       mem, exec, fav_total_passive, cav_past_passive, cav_current_passive
MEM:        sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse,      exec, fav_total_passive, cav_past_passive, cav_current_passive
EXEC:       sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem,       fav_total_passive, cav_past_passive, cav_current_passive
...
FAV_total:    sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, cav_current_passive, cav_current_past
CAV_lifetime: sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, fav_total_passive
CAV_current:  sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, fav_total_passive
CAV_past:     sex, age, edu, apoe, temporoparietal_thick_weighted, diagnosis, mmse, mem, exec, fav_total_passive
...
methodQ["CAV_lifetime_passive"] <- "~I((CAV_1 + CAV_2 + CAV_3 + CAV_4 + CAV_5 + CAV_6 + CAV_7 + CAV_8 + CAV_9 + CAV_10 + CAV_11 + CAV_12 + CAV_13 +
                                    CAV_14 + CAV_15 + CAV_16 + CAV_17 + CAV_18 + CAV_19 + CAV_20 + CAV_21 + CAV_22 + CAV_23 + CAV_24 + CAV_25) / 25)"

methodQ["CAV_past_passive"] <- "~I( (CAV_1 + CAV_2 + CAV_3 + CAV_4 + CAV_5 + CAV_6 + CAV_7 + CAV_8 + CAV_9 + CAV_10 + CAV_11 + CAV_12 + CAV_13 +
                                  CAV_14 + CAV_15 + CAV_16 + CAV_17 + CAV_18 + CAV_19 + CAV_20) /20) "
...
predQ <- as.matrix(xlsx::read.xlsx(here("P4_imputation_models.xlsx"), sheetName = "Part2_passive_matrix", row.names=1))
...
vis <- c("CAV_1", "CAV_2", "CAV_3", "CAV_4", "CAV_5", "CAV_6", "CAV_7", "CAV_8", "CAV_9", "CAV_10", "CAV_11", "CAV_12", "CAV_13", 
         ...
         "Q2", "Q3", "Q4", "Q5", "Q6", "Q7", "Q8", "Q9A", "Q9B", "Q9C", "Q9D", "Q10", 
         "FAV_total_passive",
         "I_ID", "I_sex", "I_age_q", "I_edu_VE", "APOE", "D_Q_grouped", "MMSE_q", "Z_MEM", "Z_EXEC", "temporoparietal_thick_weighted", 
         "CAV_lifetime", "CAV_current", "CAV_past", "FAV_total")
...
saveRDS(imp, here("data", "processed", "Part2_mids_m40i100.rds"))
...
```{r}
imp <- readRDS(here("data", "processed", "Part2_mids_m40i100.rds"))
...
imp_dat$CAV_lifetime_comp <- rowMeans(imp_dat[c(paste0("CAV_", 1:25))])
imp_dat$CAV_past_comp <- rowMeans(imp_dat[c(paste0("CAV_", 1:20))])
imp_dat$CAV_current_comp <- rowMeans(imp_dat[c(paste0("CAV_", 21:25))])
...
# convergence of other variables
 plot(imp, c("I_edu_VE", "APOE", "MMSE_q", "Z_MEM", "Z_EXEC"))
