...
results_folder = "results/3_mortality/models/mortality_models_sensitivity_Edu_tert_APOE"
...
datimp_cox <- datimp %>% 
  select(.imp, .id, I_ID, Time_mort, Died, TP_thick_s, Age_s, Sex_num, Edu_tert, APOE_e4, CAV_lifetime_s, CAV_past_s, CAV_current_s, FAV_s) %>% 
...
cox_CAV_lifetime_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_mort, Died) ~ TP_thick_s*CAV_lifetime_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
...
cox_CAV_past_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_mort, Died) ~ TP_thick_s*CAV_past_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
                                       p.crit=1, method="RR", direction = "BW")
...
cox_CAV_current_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_mort, Died) ~ TP_thick_s*CAV_current_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
                                       p.crit=1, method="RR", direction = "BW")
...
cox_FAV_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                              formula = Surv(Time_mort, Died) ~ TP_thick_s*FAV_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
                              p.crit=1, method="RR", direction = "BW")
