...
results_folder = "results/3_mortality/models/conversion_models_sensitivity_Edu_tert"
...
datimp_cox <- datimp %>% 
  filter(Diagnosis != "AD") %>% 
  filter(Time_conv != 0) %>% 
  select(.imp, .id, I_ID, Time_conv, Conv, TP_thick_s, Age_s, Sex_num, Edu_tert, CAV_lifetime_s, CAV_past_s, CAV_current_s, FAV_s)

# create strata datasets (needs to be done like this because of the psfmi_coxr function doesn't like categorical like this)
...
cox_CAV_lifetime_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_conv, Conv) ~ TP_thick_s*CAV_lifetime_s + Age_s + Sex_num + Edu_tert, 
...
cox_CAV_past_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_conv, Conv) ~ TP_thick_s*CAV_past_s + Age_s + Sex_num + Edu_tert, 
                                       p.crit=1, method="RR", direction = "BW")
...
cox_CAV_current_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_conv, Conv) ~ TP_thick_s*CAV_current_s + Age_s + Sex_num + Edu_tert, 
                                       p.crit=1, method="RR", direction = "BW")
...
cox_FAV_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                              formula = Surv(Time_conv, Conv) ~ TP_thick_s*FAV_s + Age_s + Sex_num + Edu_tert, 
                              p.crit=1, method="RR", direction = "BW")
