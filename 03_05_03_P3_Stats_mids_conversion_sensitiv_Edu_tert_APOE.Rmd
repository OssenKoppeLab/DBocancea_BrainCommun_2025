...
results_folder = "results/3_mortality/models/conversion_models_sensitivity_Edu_tert_APOE"
...
datimp <- complete(datmids, action="long", include = FALSE) %>% 
  ...
  mutate(Edu_tert = as.numeric(Edu_tert),
         APOE_e4 = as.numeric(APOE_e4))

# subset data to use in cox models
datimp_cox <- datimp %>% 
  filter(Diagnosis != "AD") %>% 
  filter(Time_conv != 0) %>% 
  select(.imp, .id, I_ID, Time_conv, Conv, TP_thick_s, Age_s, Sex_num, Edu_tert, APOE_e4, CAV_lifetime_s, CAV_past_s, CAV_current_s, FAV_s)


# create strata datasets (needs to be done like this because of the psfmi_coxr function doesn't like categorical variables)
datimp_cox_SCD <- datimp %>% 
  filter(Diagnosis == "SCD") %>% 
  filter(Time_conv != 0) %>% 
  select(.imp, .id, I_ID, Time_conv, Conv, TP_thick_s, Age_s, Sex_num, Edu_tert, APOE_e4, CAV_lifetime_s, CAV_past_s, CAV_current_s, FAV_s)

datimp_cox_MCI <- datimp %>% 
...
cox_CAV_lifetime_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_conv, Conv) ~ TP_thick_s*CAV_lifetime_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
...
cox_CAV_past_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_conv, Conv) ~ TP_thick_s*CAV_past_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
                                       p.crit=1, method="RR", direction = "BW")
...
cox_CAV_current_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                                       formula = Surv(Time_conv, Conv) ~ TP_thick_s*CAV_current_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
                                       p.crit=1, method="RR", direction = "BW")
...
cox_FAV_inter_s <- psfmi_coxr(data=datimp_cox, nimp=40, impvar=".imp", 
                              formula = Surv(Time_conv, Conv) ~ TP_thick_s*FAV_s + Age_s + Sex_num + Edu_tert + APOE_e4, 
                              p.crit=1, method="RR", direction = "BW")
...
Merge everything and select only the effects of interest (interaction, independent effect)
...
saveRDS(all_results, file = here(results_folder, "conversion_all_models_results.RDS"))
...
all_results <- readRDS(here(results_folder, "conversion_all_models_results.RDS"))
...
openxlsx::write.xlsx(summary_table, here(results_folder, "Summary_table_Edu_tert_APOE.xlsx"), asTable = F)
